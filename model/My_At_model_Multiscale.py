"""
  Filename       [ My_At_model_Multiscale.py ]
  PackageName    [ AtJ_DH.model ]
  Synopsis       [ AtJ Model with multiple scale input ] 
"""
...
class Dense_At(nn.Module):
    def __init__(self):
        ...
        self.refineA  = nn.Conv2d(9, 3, kernel_size=7, stride=1, padding=3)
        self.refineT  = nn.Conv2d(3, 1, kernel_size=7, stride=1, padding=3)

    def forward(self, x):
        A = torch.zeros((x.shape[0], x.shape[1] * 3, x.shape[2], x.shape[3]))
        T = torch.zeros((x.shape[0], 3, x.shape[2], x.shape[3]))

        # Looping with different scale
        for i, scale in enumerate((1, 0.5, 0.25)):
            # Downsampling
            xi = x if scale == 1.0 else self.upsample(x, scale_factor=scale)

            # Encoder: Input Shape is assumed as (512, 512)
            x1, x2, x4 = self.encoder(xi)

            # Decoder: Input Shape is assumed as (8, 8)
            Ai = self.decoder_A(xi, x1, x2, x4)
            Ti = self.decoder_T(xi, x1, x2, x4)
            Ti = self.convT(self.ResT(self.convT1(Ti)))

            # Upsampling
            if scale != 1.0:
                Ai = self.upsample(Ai, scale_factor=1/scale)
                Ti = self.upsample(Ti, scale_factor=1/scale)
