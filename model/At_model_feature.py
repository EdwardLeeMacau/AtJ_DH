...
class Dense(nn.Module):
    ...
    def forward(self, x):
        ...
        #     Image.fromarray((255 * x0[0, i].cpu().numpy()).clip(0, 255).astype(np.uint8), mode="L").save('x0_{}.png'.format(i))
        ...
        #     Image.fromarray((255 * x1[0, i].cpu().numpy()).clip(0, 255).astype(np.uint8), mode="L").save('x1_{}.png'.format(i))

        ###  32x32
        x2 = self.trans_block2(self.dense_block2(x1))

        # for i in range(x2.size(1)):
        #     Image.fromarray((255 * x2[0, i].cpu().numpy()).clip(0, 255).astype(np.uint8), mode="L").save('x2_{}.png'.format(i))

        ### 16 X 16
        x3 = self.trans_block3(self.dense_block3(x2))

        # for i in range(x3.size(1)):
        #     Image.fromarray((255 * x3[0, i].cpu().numpy()).clip(0, 255).astype(np.uint8), mode="L").save('x3_{}.png'.format(i))
        ...
        #     Image.fromarray((255 * x4[0, i].cpu().numpy()).clip(0, 255).astype(np.uint8), mode="L").save('x4_{}.png'.format(i))
        ...
        #     Image.fromarray((255 * A[0, i].cpu().numpy()).clip(0, 255).astype(np.uint8)).save('A_{}.png'.format(i))

        # for i in range(T.size(1)):
        #     Image.fromarray((255 * T[0, i].cpu().numpy()).clip(0, 255).astype(np.uint8)).save('T_{}.png'.format(i))

        # Image.fromarray((255 * A[0].cpu().numpy().transpose(1, 2, 0)).clip(0, 255).astype(np.uint8)).save('A.png')
        # Image.fromarray((255 * T[0].cpu().numpy().transpose(1, 2, 0)).clip(0, 255).astype(np.uint8)).save('T.png')
