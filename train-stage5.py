    ...
    with SummaryWriter(comment='AtJ_DH') as writer:
        for epoch in range(startepoch, epochs):
            ...
            for i, (data, target) in enumerate(dataloader, 1): 
                ...
                Rehaze_L2 = criterionMSE(rehaze, data)

                if kappa != 0:
                    rehazevgg = net_vgg(rehaze)
                    datavgg = net_vgg(data)
                    Rehaze_Lp = sum([criterionMSE(rehazeVGG, dataVGG) for (rehazeVGG, dataVGG) in zip(rehazesvgg, datavgg)])
                    loss += Rehaze_L2 + kappa * Rehaze_Lp
